// Items được cập nhật - redline (case 65$)
// Items < 65$: giữ như hiện tại
// Items >= 65$: dropPercent 10% -> 0.05% (theo giá trị - đắt hơn = thấp hơn)
// Items ≥ 300$: dropPercent thấp 0.5-5% (khó nhận)

// Helper function để random item theo dropPercent (inline để không phụ thuộc file riêng)
if (typeof window.getRandomItemByDropPercent === 'undefined') {
	window.getRandomItemByDropPercent = function(items) {
...
// Update khi page load
updateStatsDisplay();
// Update mỗi giây để simulate real-time
...
const createInfoAboutItemsInChest = () => {
	...
	for (i = 0; i < countItemsAmount; i++) {
		...
		// Hiển thị dropPercent gốc (như blaze)
...
const createItemsInChest = () => {
	...
	caseItemsBox.forEach((box) => {
		...
		// Tạo nhiều items theo dropPercent để spin hiển thị đa dạng items trong case
		const totalItemsToCreate = 300;
		for (i = 0; i < totalItemsToCreate; i++) {
			const randomItem = window.getRandomItemByDropPercent
				? window.getRandomItemByDropPercent(items2)
				: Math.floor(Math.random() * Object.keys(items2).length); // fallback random đều
...
// Helper function để kiểm tra balance SOL
const checkSolBalance = async () => {
	try {
		// Nếu đã kết nối ví, kiểm tra SOL balance thật
		if (window.wallet && window.wallet.isConnected() && window.solPriceConverter) {
			const solPrice = await window.solPriceConverter.getSolPrice();
			const requiredSol = await window.solPriceConverter.usdToSol(casePrice * casesAmount);
			const currentSolBalance = await window.wallet.getBalance();
		...
		// Fallback: kiểm tra localStorage balance (USD mock)
		const balance = parseFloat(localStorage.getItem("Balance")?.slice(0, -1) || 0);
		const required = casePrice * casesAmount;
		return { 
		...
		console.error('Lỗi kiểm tra balance:', error);
...
const setBtnText = async () => {
	...
	const balanceInfo = await checkSolBalance();
	
	if (balanceInfo.hasEnough) {
		if (balanceInfo.isSol && window.solPriceConverter) {
			// Hiển thị giá bằng SOL nếu dùng ví thật
			spinBtn.textContent = `open ${window.solPriceConverter.formatSol(balanceInfo.required)}`;
		} else {
			// Hiển thị giá bằng USD nếu dùng mock balance
...
const spinCase = async () => {
	...
	if (!balanceInfo.hasEnough || spinBtn.textContent === "spining") {
	...
	// Nếu dùng SOL thật, thực hiện thanh toán SOL trước
	if (balanceInfo.isSol && window.wallet && window.wallet.isConnected() && window.solanaPayment) {
		try {
			spinBtn.textContent = "processing..."; // Hiển thị đang xử lý
			
			// Gửi SOL payment
			const paymentResult = await window.solanaPayment.sendPayment(
				balanceInfo.required,
				window.CONFIG?.walletAddress
			);
			
			if (!paymentResult.success) {
				alert(paymentResult.error || 'Thanh toán thất bại. Vui lòng thử lại.');
				await setBtnText(); // Reset button text
				return;
			}
			
			// Lưu transaction vào Supabase nếu có (sẽ cập nhật item sau khi quay xong)
			// Lưu tạm để có transaction_id
			let transactionId = null;
			if (window.supabaseClient && window.wallet) {
				try {
					const walletAddress = await window.wallet.getPublicKey();
					const result = await window.supabaseClient.saveTransaction({
						walletAddress: walletAddress?.toString() || localStorage.getItem("walletAddress"),
						signature: paymentResult.signature,
						caseName: "Redline",
						casePriceUsd: casePrice * casesAmount,
						casePriceSol: balanceInfo.required,
						itemWon: null, // Sẽ cập nhật sau khi quay xong
						itemPrice: null
					});
					...
					console.error('Lỗi lưu transaction:', supabaseError);
					// Không block quá trình quay hòm nếu Supabase lỗi
				}
			}
			
			// Refresh balance sau khi thanh toán
			if (window.setBalance) {
				await window.setBalance();
			}
		} catch (error) {
			console.error('Lỗi thanh toán SOL:', error);
	...
	if (true) { // Đã kiểm tra balance ở trên
		...
		caseItemsBox.forEach((box) => {
			// for each case box
			// Chọn winning item theo dropPercent trước khi spin
			const winningItemIndex = window.getRandomItemByDropPercent
				? window.getRandomItemByDropPercent(items2)
				: Math.floor(Math.random() * Object.keys(items2).length);
			const winningItemId = `item${winningItemIndex}`;
			box.dataset.winningItemId = winningItemId;

			// Tìm tất cả items có id khớp với winning item trong box
			const allItemsEls = box.querySelectorAll(".case__item");
			const winningItemElements = Array.from(allItemsEls).filter(
				(item) => item.id === winningItemId
			);

			// Nếu có items khớp, tính howStrongSpin để dừng đúng item đó ở red line
			let howStrongSpin;
			if (winningItemElements.length > 0) {
				const targetItem =
					winningItemElements[
						Math.floor(Math.random() * winningItemElements.length)
					];
				const redLineX =
					box.parentElement.children[0].getBoundingClientRect().x;
				const targetItemRect = targetItem.getBoundingClientRect();
				const targetItemCenterX = targetItemRect.x + targetItemRect.width / 2;
				const currentLeft = parseFloat(box.style.left) || 0;
				const offsetNeeded = redLineX - targetItemCenterX;
				howStrongSpin = currentLeft + offsetNeeded;
				const itemWidth = targetItemRect.width;
				howStrongSpin += (Math.random() - 0.5) * itemWidth * 0.1; // ±5%
			} else {
				howStrongSpin = Math.floor(Math.random() * 5000 - 10000);
			}
			...
			// Chỉ trừ localStorage balance nếu KHÔNG dùng SOL thật
			// (SOL thật đã được trừ ở trên)
			if (!balanceInfo.isSol) {
			...
			setTimeout(() => {
				function getWinningItem() {
					const winningItemId = box.dataset.winningItemId;
					...
					// Ép item ở red line thành item đã chọn theo dropPercent (đảm bảo hiển thị/nhận trùng nhau)
					if (closestItem && winningItemId && items2[winningItemId]) {
						closestItem.id = winningItemId;
						const itemImg = closestItem.querySelector(".case__img");
						const itemName = closestItem.querySelector(".case__item-name");
						const itemSkin = closestItem.querySelector(".case__skin-name");

						if (itemImg) {
							itemImg.setAttribute("src", items2[winningItemId].imgDist);
							itemImg.setAttribute("alt", items2[winningItemId].name);
						}
						if (itemName) itemName.textContent = items2[winningItemId].weapon;
						if (itemSkin) {
							itemSkin.textContent = items2[winningItemId].skin;
							itemSkin.className =
								"case__skin-name " + items2[winningItemId].color + "-text";
						}
						closestItem.className =
							"case__item case__item" +
							box.id +
							" " +
							items2[winningItemId].color +
							"-drop";
					}
		...
		// Lưu case opening vào Supabase sau khi quay xong
		setTimeout(async () => {
			if (winningItems.length > 0) {
				const winningItem = winningItems[0]; // Lấy item đầu tiên
				
				// Lưu vào Supabase nếu có
				if (window.supabaseClient && window.wallet && winningItem && items2[winningItem.id]) {
					try {
						const walletAddress = await window.wallet.getPublicKey();
						await window.supabaseClient.saveCaseOpening({
							walletAddress: walletAddress?.toString() || localStorage.getItem("walletAddress"),
							caseName: "Redline",
							itemWonId: items2[winningItem.id].id,
							itemWonName: items2[winningItem.id].name,
							itemPrice: items2[winningItem.id].price
						});
					} catch (error) {
						console.error('Lỗi lưu case opening:', error);
						// Không block UI nếu Supabase lỗi
					}
				}
			}
		}, 6000); // Sau khi animation xong
	}
};
