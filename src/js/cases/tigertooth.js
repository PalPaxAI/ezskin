// Items được cập nhật - tigertooth (case 400$)
// Items < 400$: dropPercent 90% → 50% (theo giá trị - rẻ hơn = cao hơn)
// Items > 400$: dropPercent 10% → 0.05% (theo giá trị - đắt hơn = thấp hơn)
...
const createInfoAboutItemsInChest = () => {
	...
	for (i = 0; i < countItemsAmount; i++) {
		...
		// Hiển thị dropPercent gốc (như luckylore/redline)
...
const createItemsInChest = () => {
	...
	caseItemsBox.forEach((box) => {
		...
		// Tạo items theo tỉ lệ dropPercent để đảm bảo tỉ lệ hiển thị khớp với tỉ lệ nhận được
		// Tổng số items: ít nhất 300 để animation mượt mà
		const totalItemsToCreate = 300;
		
		for (i = 0; i < totalItemsToCreate; i++) {
			// Random item theo dropPercent - đảm bảo tỉ lệ xuất hiện khớp với dropPercent
...
const spinCase = () => {
	...
	if (
		...
		caseItemsBox.forEach((box) => {
			...
			// Chọn winning item theo dropPercent TRƯỚC KHI animation bắt đầu
			// Để đảm bảo tỉ lệ ra items khớp với dropPercent hiển thị
			...
			// Lưu winning item id vào box để sử dụng sau
			...
			const winningItemElements = Array.from(allItemsEls).filter(item => item.id === winningItemId);
			
			// Nếu có items khớp, chọn một item ngẫu nhiên trong số đó
			// Sau đó tính toán vị trí để item đó dừng ở red line
			let howStrongSpin;
			if (winningItemElements.length > 0) {
				// Chọn một item ngẫu nhiên trong số các items khớp
				const targetItem = winningItemElements[Math.floor(Math.random() * winningItemElements.length)];
				
				// Lấy vị trí red line
				const redLineX = box.parentElement.children[0].getBoundingClientRect().x;
				
				// Lấy vị trí hiện tại của target item
				const targetItemRect = targetItem.getBoundingClientRect();
				const targetItemCenterX = targetItemRect.x + targetItemRect.width / 2;
				
				// Tính toán offset cần thiết để item dừng ở red line
				...
				// Thêm một chút random nhỏ để animation tự nhiên hơn (nhưng vẫn đảm bảo dừng đúng item)
				const itemWidth = targetItemRect.width;
				howStrongSpin += (Math.random() - 0.5) * itemWidth * 0.1; // ±5% của item width
			} else {
				// Fallback: random như cũ nếu không tìm thấy item
			...
			// after animation end, get winning item
			setTimeout(() => {
				function getWinningItem() {
					// Lấy winning item id đã chọn trước đó
					const winningItemId = box.dataset.winningItemId;
					
					// Tìm item gần nhất với red line sau animation
					const redLineX =
					...
					// Thay đổi item gần nhất thành winning item đã chọn theo dropPercent
					if (closestItem && winningItemId && items2[winningItemId]) {
						closestItem.id = winningItemId;
						// Cập nhật nội dung của item để khớp với winning item
						...
						if (itemName) {
							itemName.textContent = items2[winningItemId].weapon;
						}
						if (itemSkin) {
							itemSkin.textContent = items2[winningItemId].skin;
							// Cập nhật class màu
							itemSkin.className = "case__skin-name " + items2[winningItemId].color + "-text";
						}
						// Cập nhật class màu của item
						closestItem.className = "case__item case__item" + box.id + " " + items2[winningItemId].color + "-drop";
					}
