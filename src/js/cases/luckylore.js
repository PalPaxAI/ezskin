// Items được cập nhật - tỉ lệ dựa trên giá so với case (200$)
// Items < 150$: dropPercent 30-90% (theo giá trị - rẻ hơn = cao hơn)
// Items 150$ - 200$: dropPercent 10-30% (bình thường)
// Items > 200$ - 350$: dropPercent 5% - 1% (theo giá trị - đắt hơn = thấp hơn)
// Items > 350$: dropPercent 4.5% - 0.05% (theo giá trị - đắt hơn = thấp hơn)
const items2 = {
	item0: {
		weapon: "M4A4",
		name: "M4A4 Mainframe",
...
const spinCase = () => {
	...
	if (
		...
		caseItemsBox.forEach((box) => {
			...
			const allItems = box.querySelectorAll(".case__item");
			const winningItemElements = Array.from(allItems).filter(item => item.id === winningItemId);
