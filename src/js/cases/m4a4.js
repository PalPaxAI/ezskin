...
const createInfoAboutItemsInChest = () => {
	...
	const dropBox = document.querySelector(".case__drop-box"); // container for info about all items in case
		...
		// Tính tổng dropPercent để tính tỉ lệ phần trăm thực tế
	const totalDropPercent = Object.keys(items2).reduce((sum, key) => {
		return sum + (items2[key].dropPercent || 0);
	}, 0);
	...
	for (i = 0; i < countItemsAmount; i++) {
		...
		dropItemImg.setAttribute("alt", items2[`item${i}`].name);
				...
				// Tính tỉ lệ phần trăm thực tế dựa trên tổng dropPercent
		const actualPercent = totalDropPercent > 0 
			? ((items2[`item${i}`].dropPercent / totalDropPercent) * 100).toFixed(2)
			: items2[`item${i}`].dropPercent;
		dropItemPercent.textContent = actualPercent + "%"; // set textcontent
