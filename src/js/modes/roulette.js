	...
	if (
		...
		switch (this.id) {
			case "redBtn":
				...
				let userAvatarRed = localStorage.getItem("avatar");
				if (!userAvatarRed || 
					userAvatarRed.trim() === "" || 
					userAvatarRed.includes("pravatar.cc") || 
					userAvatarRed === "ad" ||
					(!userAvatarRed.startsWith("http") && !userAvatarRed.startsWith("./") && !userAvatarRed.startsWith("../") && !userAvatarRed.startsWith("/"))) {
					userAvatarRed = "../dist/img/avatars/avatar.png";
					localStorage.setItem("avatar", userAvatarRed);
				}
				itemImgRed.setAttribute("src", userAvatarRed);
			...
			case "blackBtn":
				...
				let userAvatarBlack = localStorage.getItem("avatar");
				if (!userAvatarBlack || 
					userAvatarBlack.trim() === "" || 
					userAvatarBlack.includes("pravatar.cc") || 
					userAvatarBlack === "ad" ||
					(!userAvatarBlack.startsWith("http") && !userAvatarBlack.startsWith("./") && !userAvatarBlack.startsWith("../") && !userAvatarBlack.startsWith("/"))) {
					userAvatarBlack = "../dist/img/avatars/avatar.png";
					localStorage.setItem("avatar", userAvatarBlack);
				}
				itemImgBlack.setAttribute("src", userAvatarBlack);
			...
			case "greenBtn":
				...
				let userAvatarGreen = localStorage.getItem("avatar");
				if (!userAvatarGreen || 
					userAvatarGreen.trim() === "" || 
					userAvatarGreen.includes("pravatar.cc") || 
					userAvatarGreen === "ad" ||
					(!userAvatarGreen.startsWith("http") && !userAvatarGreen.startsWith("./") && !userAvatarGreen.startsWith("../") && !userAvatarGreen.startsWith("/"))) {
					userAvatarGreen = "../dist/img/avatars/avatar.png";
					localStorage.setItem("avatar", userAvatarGreen);
				}
				itemImgGreen.setAttribute("src", userAvatarGreen);
...
// create random players - DISABLED: chỉ hiển thị người dùng thật, không tạo bot
const createRandomPlayers = () => {
	// Bot players đã bị vô hiệu hóa để chỉ hiển thị người dùng thật
	return;
};
