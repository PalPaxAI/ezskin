			...
			for (i = 0; i < players; i++) {
				...
				if (i === 0) {
					...
					if (!userAvatar || 
						...
						// File battles.html luôn ở subfolder modes/
						userAvatar = "../dist/img/avatars/avatar.png";
						localStorage.setItem("avatar", userAvatar);
					}
					battleImg.setAttribute("src", userAvatar);
					battleName.textContent = localStorage.getItem("nickname");
					battleBox.style.order = "-1";
				} else {
					// Bot players đã bị vô hiệu hóa - chỉ hiển thị người dùng thật
					// Ẩn các battle boxes của bot
...
const winningAnimationJackpot = (players) => {
	...
	// Bot players đã bị vô hiệu hóa - chỉ hiển thị người dùng thật (player0)
	if (players.length >= 1 && players[0].parentElement.parentElement.parentElement.id === "player0") {
		for (i = 0; i < 100; i++) {
			...
			playerAvatar.setAttribute("src", userAvatar);
...
function incrementBattles() {
	...
	if (window.mockStats) {
		window.mockStats.incrementBattles();
		updateStatsDisplay();
	}
}
