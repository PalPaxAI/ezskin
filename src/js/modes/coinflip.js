...
const spinCoin = () => {
	...
	// Random kết quả với tỉ lệ thắng thấp (~20%) cho bên người chơi chọn
	let result = "heads"; // CT
	const winChance = 0.2; // 20% win
	const rnd = Math.random();

	if (bet === "ct") {
		// Người chơi chọn CT → chỉ 20% ra heads, 80% ra tails
		result = rnd < winChance ? "heads" : "tails";
	} else if (bet === "t") {
		// Người chơi chọn T → chỉ 20% ra tails, 80% ra heads
		result = rnd < winChance ? "tails" : "heads";
	} else {
		// Fallback nếu chưa chọn bên (không nên xảy ra) → 50/50
		result = rnd < 0.5 ? "heads" : "tails";
	}
	...
	if (coin) {
		...
		coin.offsetHeight;
		coin.classList.add(result === "heads" ? "coin--heads" : "coin--tails");
	}
...
const countTotal = () => {
	const allItems = document.querySelectorAll(".coinflip__lastdropsbox .coinflip__item");
	totalAmount = 0;
	
	allItems.forEach((item) => {
		if (item.children && item.children[2]) {
			const amountText = item.children[2].textContent;
			const parsed = parseFloat(amountText);
			if (!isNaN(parsed)) {
				totalAmount = parseFloat(totalAmount) + parsed;
			}
		}
	});
